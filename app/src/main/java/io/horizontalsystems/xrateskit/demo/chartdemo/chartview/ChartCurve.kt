...
class ChartCurve(private val shape: RectF, private val config: ChartConfig) {
    ...
    fun init(points: List<ChartPointFloat>, startTimestamp: Long, endTimestamp: Long, baseCurrency: Currency?) {
    ...
    private fun format(value: Float): String {
        ...
        val fiatBigNumber = "1000".toBigDecimal()
        val maxFraction = if (config.valueScale == 0 || currencyValue.value > fiatBigNumber) null else config.valueScale

        return App.numberFormatter.formatForRates(currencyValue, trimmable = true, maxFraction = maxFraction) ?: ""
    }

    class Coordinate(val x: Float, val y: Float, val point: ChartPointFloat)
}
